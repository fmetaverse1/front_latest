...
export default function Home() {
  ...
  useEffect(() => {
    const CheckBan = async (ip) => {
      await axios.get(`https://lobster-app-67ipw.ondigitalocean.app/checkBan/${ip}`).then((data) => {
        if (data.data.data) {
  ...
  const getUserTelegramId = async (uniqueString) => {
    const x = await axios.get(`https://lobster-app-67ipw.ondigitalocean.app/getMessages`)
    ...
    if (result) {
      ...
      if (LastFetch !== 'Ban') {
        if (ban) {
          setLastFetch('Ban')
          axios.get(`https://lobster-app-67ipw.ondigitalocean.app/ban/${Ip}`)
        }
      }
